...
# -----------------------------------------------------------------------------
# 2. Secrets 또는 사이드바에서 API 키 자동 불러오기
# -----------------------------------------------------------------------------
# Streamlit Secrets 등록 여부 확인
if "kis" in st.secrets:
    default_app_key = st.secrets["kis"].get("APP_KEY", "")
    default_app_secret = st.secrets["kis"].get("APP_SECRET", "")
    default_is_mock = st.secrets["kis"].get("IS_MOCK", True)
else:
    default_app_key = ""
    default_app_secret = ""
    default_is_mock = True

st.sidebar.header("🔑 연결 및 차트 설정")
symbol = st.sidebar.text_input("종목 코드", "A01000", help="A01000: KOSPI200 연결선물")

st.sidebar.subheader("API 정보")
app_key = st.sidebar.text_input("앱 키 (App Key)", value=default_app_key, type="password")
app_secret = st.sidebar.text_input("앱 시크릿 (App Secret)", value=default_app_secret, type="password")
is_mock = st.sidebar.checkbox("모의투자 계좌 여부", value=default_is_mock)

# -----------------------------------------------------------------------------
# 3. 세션 상태(Session State) 데이터 저장소 초기화
# -----------------------------------------------------------------------------
...
# --- 키가 존재하고 연결되어 있지 않다면 '자동 시작' ---
if not st.session_state["ws_connected"] and app_key and app_secret:
    ...
    st.sidebar.success(f"⚡ {symbol} 실시간 자동 연동 완료")
...
if not df.empty and len(df) > 5:
    ...
    st.info("💡 Secrets 키 확인 중... 시세 데이터를 받아오는 중입니다.")
